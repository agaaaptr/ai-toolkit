...
2. **(patch/minor/major)** Edit the lib `package.json` to the new **plain** version; prepend a **Keep a Changelog** entry to `CHANGELOG.md` — heading `## [<new plain version>] - <YYYY-MM-DD>`, a `> Published as: \`<tag>\` (<branch>) — see VERSIONS.md for per-env status.` line, then the session's changes grouped under `### Added` / `### Changed` / `### Fixed` / `### Removed` (omit empty sections); commit both: `chore: bump version to <new version>`. **(next-pre-release)** leave `package.json` unchanged; optionally append a `Published as: <tag>` line to the existing version's CHANGELOG entry (confirm with user).
